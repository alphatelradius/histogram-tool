...
function write($phpPowerPoint, $filename, $writers)
{
	...
	foreach ($writers as $writer => $extension) {
		...
		if (!is_null($extension)) {
			...
			$xmlWriter->save(public_path() . "/{$filename}.{$extension}");
			rename(public_path() . "/{$filename}.{$extension}", public_path() . "/results/{$filename}.{$extension}");
		} else {
			$result .= ' ... NOT DONE!';
		}
		$result .= EOL;
...
function createTemplatedSlide(PhpOffice\PhpPowerpoint\PhpPowerpoint $objPHPPowerPoint)
{
	...
	$shape->setName('PHPPowerPoint logo')
		...
		->setPath(public_path('resources/phppowerpoint_logo.gif'))
		->setHeight(36)
		->setOffsetX(10)
		->setOffsetY(10);
...
// Save file
 write($objPHPPowerPoint, basename(__FILE__, '.php'), $writers);
//if (!CLI) {
