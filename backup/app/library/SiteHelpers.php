...
class SiteHelpers {
    ...
    public static function langOption() {
        ...
        foreach ($lang as $value) {
            ...
            if (is_dir(app_path() . '/lang/' . $value)) {
                $fp = file_get_contents(app_path() . '/lang/in/info.json');
    ...
    public static function themeOption() {
        ...
        foreach ($lang as $value) {
            ...
            if (is_dir(app_path() . '/views/layouts/' . $value)) {
                $fp = file_get_contents(app_path() . '/views/layouts/pelayanan/info.json');
    ...
    public static function countalbumlist($jenis, $dik) {
        $data = Media::join('album', 'media.album_id', '=', 'album.id')
                ->where('media.tipe', $jenis)
                ->where('media.album_id', $dik)
                ->count();
        // $data = Media::where('album_id',$jenis)->count();
        return $data;
    }

    public static function getGrupName($gid) {
        $grup = DB::table('tb_groups')
                ->where('group_id', '=', $gid)
                ->first();
        return $grup->name;
    }

    public static function cekpulsa($operator = '') {
        $pulsa = DB::table('pulsa')->where('operator','=',$operator)->OrderBy('id','desc')->limit(1);
        return $pulsa->first()->pulsa;
    }
